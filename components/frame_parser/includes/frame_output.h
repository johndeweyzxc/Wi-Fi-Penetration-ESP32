 ...
 * authentication and copies it into the buffer
 * @param *char_buff buffer to store output
...
/**
 * @brief This function is used to format the key ID and key RSC and copies it
 * into the buffer
 * @param *char_buff buffer to store output
 * @param *b pointer to the ID or RSC
 */
void put_id_or_rsc_in_buff(char *char_buff, uint8_t *b);

/**
 * @brief Formats the key data which is the data after the MIC or PMKID and
